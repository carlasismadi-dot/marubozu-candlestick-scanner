...
import { Bell, Volume2, VolumeX, AlertCircle, TrendingUp, TrendingDown, Clock } from 'lucide-react';
  ...
  const playChime = (type: 'bullish' | 'bearish') => {
    ...
    try {
      ...
      if (type === 'bullish') {
        ...
        playTone(329.63, now + 0.1, 0.4);
        playTone(392.00, now + 0.2, 0.4);
        playTone(523.25, now + 0.3, 0.6);
      } else {
        playTone(392.00, now,       0.4);
        playTone(311.13, now + 0.1, 0.4);
        playTone(261.63, now + 0.2, 0.6);
      }
    } catch (e) {}
  };
  ...
  useEffect(() => {
    ...
    coins.forEach(coin => {
      (['30m', '4h', '1d'] as const).forEach(tf => {
        // Keys now use interval string directly: 30m / 4h / 1d
        const candles = ohlcRecords[`${coin.id}_${tf}`] || [];
    ...
    if (newAlerts.length > 0) {
      ...
      if (triggeredSound) playChime(lastTriggerType);
    }
    ...
    <div id="notification-center-section" className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col h-full justify-between">
      ...
      <div className="mt-4 pt-3 border-t border-slate-800/60 flex items-center justify-between text-[10px] text-slate-500 font-mono">
        <span className="flex items-center gap-1">
          <Clock className="w-3 h-3 text-blue-500" /> Data Source:
        </span>
        <span className="text-slate-400 font-bold">Binance WebSocket • Live</span>
      </div>
